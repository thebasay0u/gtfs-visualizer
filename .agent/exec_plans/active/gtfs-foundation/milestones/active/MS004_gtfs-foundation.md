...
Provide the first consumer-facing interface above validated GTFS artifacts. At the end of this milestone, a developer should be able to query route, trip, and stop detail from an MS003 artifact directory without re-reading the source feed.

## Definition of Done

- [ ] Query artifacts load from valid and warning-only bundles
- [ ] Route, trip, and stop queries return locked JSON response shapes
- [ ] Query failures use explicit artifact and lookup error classes
- [ ] CLI prints query errors to stderr and returns exit code `1`
- [ ] Tests cover loader, service, and CLI behavior
